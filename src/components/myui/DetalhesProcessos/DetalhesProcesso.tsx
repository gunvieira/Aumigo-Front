// src/components/myui/DetalhesProcessos/DetalhesProcesso.tsx
...
interface DetalhesProcessoProps {
    children: ReactNode;
    processo: IProcesso;
    onUpdate: () => Promise<void>; // Ajustado para indicar que pode ser uma Promise
}
...
export default function DetalhesProcesso({ children, processo, onUpdate }: DetalhesProcessoProps) {
    ...
    const handleUpdateStatus = async (novoStatus: 'APROVADO' | 'RECUSADO') => {
        ...
        setError(null);
        try {
            // Lógica da API (descomente quando estiver pronto)

            await axios.post('http://localhost:8080/animais', {
                idAcoao: processo.idAdocao,
                status: novoStatus
            });

            console.log('Status que seria enviado para a API:', novoStatus);
            await onUpdate(); // Atualiza a lista na tela principal
            setOpen(false);   // Fecha o modal somente em caso de sucesso
        } catch (err) {
    ...
    const renderActionFooter = () => {
        if (processo.statusAdocao === 'PENDENTE') {
            return (
                <div className="flex flex-row justify-center gap-5 mt-6">
                    ...
                    <BotaoEntrar
                        ...
                        isLoading={isSubmitting}
                        // Idealmente, use uma variante 'destructive' se o seu componente de botão tiver
        ...
        return (
            <div className="mt-6 text-center">
                <p className="text-gray-600 font-medium bg-gray-100 p-3 rounded-md">
                    Este processo foi finalizado com o status: <strong>{processo.statusAdocao}</strong>
                </p>
            </div>
        );
    };
                    ...
                    <div className="bg-white rounded-lg p-2 w-full">
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-4">
                                {/* 2. Acesso seguro aos dados com fallback */}
                                <img
